...
plt.figure()
plt.title('Histograma do erro entre o sinal reconstruido e previsão')
plt.hist( (y_original.iloc[STEPS_FORECAST:].price - reco.value), color='red', alpha=0.5, label='erro entre o sinal original e sinal previsto reconstruído' )
plt.hist( (y_original.iloc[STEPS_FORECAST:].price \
                         - (y_pred[STEPS_FORECAST:] + reco).value), color='green', alpha=0.5, label='erro entre o sinal original e sinal previsto reconstruído com rede para diminuir erro')
plt.legend()
