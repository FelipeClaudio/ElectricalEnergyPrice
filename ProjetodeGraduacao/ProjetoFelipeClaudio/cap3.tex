...
[colocar imagem de neurônio biológico]
...
[colocar imagem de neurônio artificial]
...
[colocar figura da função logística]
...
[Colocar figura da tangente hiperbólica]
...
\paragraph{}Para que a rede consiga de fato "aprender" com os dados é necessário realizar o treinamento da rede neural. Para que o mesmo seja bem sucedido é necessário atentar para alguns fatores como: 
\begin{itemize}
...
\paragraph{}Nas camadas seguintes utiliza-se a saída da camada anterior ($M$ saídas )com entrada na camada atual (camada $H$ com $P$ neurônios). 
\begin{equation}
Y^{(p)} = g(\sum_{i = 0}^{M} W_{pi}^{(H)} \cdot Y_{i}^{(H-1)})
\end{equation}
